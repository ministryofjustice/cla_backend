...
from knowledgebase.models import Article, TelephoneNumber, ArticleCategory, \
    ArticleCategoryMatrix
...
class Command(BaseCommand):
    ...
    def set_helpline(self, obj, value):
        for n in value.split('\n'):
            ...
            if number:
                t, created = TelephoneNumber.objects.get_or_create(
                    article=obj,
                    name=name,
                    number=number,
                )
    ...
    def set_categories(self, obj, value):
        for cn in value.split(','):
            try:
                ...
                continue
            acm, created = ArticleCategoryMatrix.objects.get_or_create(
                article=obj,
                article_category=ac
            )
