...
class PPSerializer(object):
    ...
    @property
    def id(self):
        return base64.b64encode(".".join([self.time_string, self.period, self._variable]))

    @property
    def headers(self):
        return {"content-type": "application/json", "Authorization": "Bearer %s" % settings.PERFORMANCE_PLATFORM_TOKEN}
    ...
    def from_time(self):
        ...
        return day_start - datetime.timedelta(days=day_start.weekday()) - datetime.timedelta(weeks=1)
    ...
    def _data(self):
        data = {"_id": self.id, "_timestamp": self.time_string, "period": self.period, "count": self.count}
...
class TransactionsByChannelTypeSerializer(PPSerializer):
    ...
    def _get_cases(self):
        return Case.objects.filter(source=self.channel, created__gte=self.from_time, created__lte=self.to_time)

    def _get_eligibility_checks(self):
        return EligibilityCheck.objects.filter(created__gte=self.from_time, created__lte=self.to_time)

    def _get_diagnosis_traversals(self):
        return DiagnosisTraversal.objects.filter(created__gte=self.from_time, created__lte=self.to_time)
    ...
    def _count(self):
        ...
        logger.info("Successful performance calc: %s, %s: %s" % (self.channel_type, self.channel, val))
