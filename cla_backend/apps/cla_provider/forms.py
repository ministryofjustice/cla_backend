...
class SplitBaseCaseForm(BaseCaseLogForm):
    LOG_EVENT_KEY = "split_case"
    SKIP_IS_PART_OF_SPLIT = False
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(SplitBaseCaseForm, self).__init__(*args, **kwargs)
    ...
    def validate_provider(self, non_field_errors):
        if self.case.provider != self.request.user.staff.provider:
            non_field_errors.append("Only Providers assigned to the Case can split it.")

    # validate that category exists
    def validate_case_has_category(self, non_field_errors):
        if not self.case.eligibility_check or not self.case.eligibility_check.category:
            non_field_errors.append("The selected Case doesn't have any category associated.")

    # validate that the case has not been previously split already
    def validate_case_split_state(self, non_field_errors):
        if not self.SKIP_IS_PART_OF_SPLIT and self.case.is_part_of_split():
            non_field_errors.append("This Case has already been split or it has been generated by another Case")

    # #### CATEGORY #####
    def validate_category(self, cleaned_data):
        if self.SKIP_SAME_CATEGORY:
            return
    ...
    # #### MATTER TYPES #####
    def validate_matter_types(self, cleaned_data):
        category = cleaned_data.get("category")
        matter_type1 = cleaned_data.get("matter_type1")
        matter_type2 = cleaned_data.get("matter_type2")
    ...
    def validate_internal(self, cleaned_data):
        ...
        if internal and not can_deal:
            ...
            cleaned_data.pop("internal", None)

    def clean(self):
        """
        Shared cleaning logic. Subclasses can change behaviour by setting:
          - SKIP_IS_PART_OF_SPLIT = True  (MCC form)
          - SKIP_SAME_CATEGORY = True     (MCC form)
        """
        cleaned_data = super(SplitBaseCaseForm, self).clean()

        if self._errors:
            return cleaned_data

        non_field_errors = []
        self.validate_provider(non_field_errors)
        self.validate_case_has_category(non_field_errors)
        self.validate_case_split_state(non_field_errors)

        if non_field_errors:
            self._errors[NON_FIELD_ERRORS] = ErrorList(non_field_errors)
            return cleaned_data

        self.validate_category(cleaned_data)
        self.validate_matter_types(cleaned_data)
        self.validate_internal(cleaned_data)
        return cleaned_data
    ...
    def get_kwargs(self):
        kwargs = super(SplitBaseCaseForm, self).get_kwargs()
...
class SplitMCCCaseForm(SplitBaseCaseForm):
    # Skip the 'already split' check and allow same-category splits
    SKIP_IS_PART_OF_SPLIT = True
