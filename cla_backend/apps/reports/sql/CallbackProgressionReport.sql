SELECT
    ...
    (log.context::jsonb->>'requires_action_at')::timestamptz::date AS callback_date,
    TO_CHAR((log.context::jsonb->>'requires_action_at')::timestamptz, 'HH24:MI') AS booking_slot,
    c.source,
...
WHERE log.code IN ('CB1', 'CB2', 'CB3', 'CB4')
  AND (log.context::jsonb->>'requires_action_at')::timestamptz::date >= %(from_date)s
  AND (log.context::jsonb->>'requires_action_at')::timestamptz::date < %(to_date)s
