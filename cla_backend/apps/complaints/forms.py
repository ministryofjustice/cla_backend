...
class ComplaintLogForm(EventSpecificLogForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ComplaintLogForm, self).__init__(*args, case=self.complaint.eod.case, **kwargs)
    ...
    def get_event_code_choices(self):
        return [(code, details.get("description", code)) for (code, details) in self.get_operator_code_objects()]

    def clean_resolved(self):
        resolved = self.cleaned_data.get("resolved")
        if self.cleaned_data.get("event_code") == "COMPLAINT_CLOSED" and resolved not in (True, False):
