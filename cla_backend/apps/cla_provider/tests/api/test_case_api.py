...
class BaseCaseTestCase(CLAProviderAuthBaseApiTestMixin, BaseFullCaseAPIMixin, APITestCase):
    ...
    def get_extra_search_make_recipe_kwargs(self):
        return {"provider": self.provider, "requires_action_by": REQUIRES_ACTION_BY.PROVIDER}
    ...
    def make_resource(self, **kwargs):
        ...
        return super(BaseCaseTestCase, self).make_resource(**kwargs)
...
class FilteredSearchCaseTestCase(BaseCaseTestCase):
    def setUp(self):
        ...
        self.cases = [
            # obj1 on operator queue => always ignored
            make_recipe("legalaid.case", reference="ref1", provider=None),
            # obj2 assigned to different provider => always ignored
            make_recipe("legalaid.case", reference="ref2", provider=other_provider),
            # obj3 assigned to provider => new
            make_recipe(
                "legalaid.case",
                reference="ref3",
            ...
            # obj5 assigned to provider but marked as 'IRCB' => always ignored
            make_recipe("legalaid.case", reference="ref5", provider=self.provider, outcome_code="IRCB"),
            # obj6 accepted by provider => accepted
            make_recipe(
                "legalaid.case",
                reference="ref6",
...
class ReopenCaseTestCase(ImplicitEventCodeViewTestCaseMixin, BaseCaseTestCase):
    ...
    def test_fails_if_case_not_closed(self):
        ...
        response = self.client.post(
            self.url, data={"notes": "lorem ipsum"}, format="json", HTTP_AUTHORIZATION="Bearer %s" % self.token
        )
...
class SplitCaseTestCase(ImplicitEventCodeViewTestCaseMixin, BaseCaseTestCase):
    ...
    def get_url(self, reference=None):
        reference = reference or self.resource.reference
        return reverse("cla_provider:case-split", args=(), kwargs={"reference": reference})
