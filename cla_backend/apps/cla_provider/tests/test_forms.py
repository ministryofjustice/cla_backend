...
class CloseCaseFormTestCase(BaseCaseLogFormTestCaseMixin, TestCase):
    ...
    def test_DREFER_save_successfull(self):
        ...
        self.assertEqual(case.provider_closed, None)
        self._test_save_successfull(case=case, data={"is_debt_referral": True, "notes": "lorem ipsum"})
...
class SplitCaseFormTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.provider = make_recipe(
            "cla_provider.provider", law_category=[self.cat1_data.category, self.cat2_data.category]
        )
        self.request = mock.MagicMock(user=mock.MagicMock(staff=mock.MagicMock(provider=self.provider)))
        self.eligibility_check = make_recipe("legalaid.eligibility_check", category=self.cat1_data.category)
    ...
    def test_invalid_if_case_doesnt_have_category(self):
        def test_invalid(case):
            form = SplitCaseForm(case=self.case, request=self.request, data=self.get_default_data())
            self.assertFalse(form.is_valid())
            self.assertDictEqual(form.errors, {"__all__": ["The selected Case doesn't have any category associated."]})
    ...
    def test_invalid_case_of_different_provider(self):
        def test_invalid(case):
            form = SplitCaseForm(case=case, request=self.request, data=self.get_default_data())
            self.assertFalse(form.is_valid())
            self.assertDictEqual(form.errors, {"__all__": ["Only Providers assigned to the Case can split it."]})
    ...
    def test_invalid_basic(self):
        ...
        form = SplitCaseForm(
            ...
            data=self.get_default_data(category="invalid", matter_type1="invalid", matter_type2="invalid"),
        )
        self.assertFalse(form.is_valid())
    ...
    def test_invalid_same_category(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertDictEqual(form.errors, {"category": ["Please choose a different category or law."]})
    ...
    def test_invalid_internal_provider_cant_deal_with_category(self):
        ...
        self.assertDictEqual(
            form.errors,
            {"internal": ["Internal can only be chosen if you can deal with the selected Category of Law."]},
        )
    ...
    def test_cannot_split_again(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertDictEqual(
            form.errors, {"__all__": ["This Case has already been split or it has been generated by another Case"]}
        )
    ...
    def _test_save_with_outcome(self, internal):
        ...
        self.assertEqual(
            ...
            "Split case created and referred %s" % ("internally" if internal else "externally"),
        )
...
class SplitMCCCaseFormTestCase(TestCase):
    ...
    def test_mcc_allows_same_category(self):
        ...
        """
        # Choose same category as case eligibility
        data = self.get_default_data(category=self.cat1.category.code, matter_type1=self.cat1.matter_type1.code,
                                     matter_type2=self.cat1.matter_type2.code, internal=False)

        form = SplitMCCCaseForm(case=self.case, request=self.request, data=data)
        # Should be valid (where SplitCaseForm would be invalid)
    ...
    def test_splitcase_still_prevents_same_category_and_repeated_split(self):
        ...
        """
        # Same category should be invalid on SplitCaseForm
        data_same = self.get_default_data(category=self.cat1.category.code,
        ...
        # Split the child with SplitCaseForm, should be invalid
