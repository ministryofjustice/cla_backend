...
class Migrater(object):
    def __init__(self, Case, Log, Staff):
        self.Case = Case
        self.Log = Log
        self.Staff = Staff
        self.warnings = []

    def _get_created(self, case, log, action):
        staff = self.Staff.objects.get(user_id=log.created_by_id)
        if not staff.provider_id == case.provider_id:
            self.warnings.append(
                "There might be something wrong with case %s. Assigned to %s but %s by provider %s. Set it to None" % (
                    case.reference, case.provider_id, action, staff.provider_id
                )
            )
            return None
        return log.created

    def _get_dates(self, case):
        logs = self.Log.objects.filter(case=case).order_by('-created')
        accepted_at = None
        closed_at = None
        for log in logs:
            if log.code == 'SPOP':
                accepted_at = self._get_created(case, log, 'accepted')
                break

            if log.code in ['MIS-MEANS', 'MIS-OOS', 'CLSP']:
                closed_at = self._get_created(case, log, 'closed')

        # if 'provider' not in (case.requires_action_by or ''):
        #     print "\n\n%s - %s" % (accepted_at, closed_at)
        #     print case.requires_action_by
        #     print '\n'.join(['%s - %s' % (cc.code, cc.created) for cc in logs if cc.code not in ['CASE_VIEWED']])
        return (accepted_at, closed_at)

    def migrate(self):
        migrated = 0
        for case in self.Case.objects.filter(provider__isnull=False):
            accepted, closed = self._get_dates(case)
            case.provider_accepted = accepted
            case.provider_closed = closed
            case.save(update_fields=['provider_accepted', 'provider_closed'])
            migrated += 1
        print 'Migrated: %s' % migrated
        print 'warnings: %s' % self.warnings
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        migrater = Migrater(orm.Case, orm['cla_eventlog.log'], orm['cla_provider.staff'])
        migrater.migrate()
    ...
    models = {
        ...
        u'cla_provider.staff': {
            'Meta': {'unique_together': "(('chs_organisation', 'chs_user'),)", 'object_name': 'Staff'},
            'chs_organisation': ('django.db.models.fields.CharField', [], {'default': "'70011ed6-ff7c-4d7f-98a2-e8352514c18e'", 'max_length': '500', 'null': 'True', 'blank': 'True'}),
            'chs_password': ('django.db.models.fields.CharField', [], {'max_length': '500', 'null': 'True', 'blank': 'True'}),
            'chs_user': ('django.db.models.fields.CharField', [], {'default': "'984c389b-5812-4d56-ac55-c5fe966e9c5b'", 'max_length': '500', 'null': 'True', 'blank': 'True'}),
            'created': ('model_utils.fields.AutoCreatedField', [], {'default': 'datetime.datetime.now'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'legalaid.case': {
            ...
            'modified': ('model_utils.fields.AutoLastModifiedField', [], {'default': 'datetime.datetime.now'}),
            'notes': ('django.db.models.fields.TextField', [], {'blank': 'True'}),
            'outcome_code': ('django.db.models.fields.CharField', [], {'max_length': '50', 'null': 'True', 'blank': 'True'}),
            ...
            'provider_accepted': ('django.db.models.fields.DateTimeField', [], {'null': 'True', 'blank': 'True'}),
            'provider_closed': ('django.db.models.fields.DateTimeField', [], {'null': 'True', 'blank': 'True'}),
        ...
        u'legalaid.casenoteshistory': {
            'Meta': {'ordering': "['-created']", 'object_name': 'CaseNotesHistory'},
            'case': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['legalaid.Case']"}),
            'created': ('model_utils.fields.AutoCreatedField', [], {'default': 'datetime.datetime.now'}),
            'created_by': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['auth.User']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'legalaid.personaldetails': {
            ...
            'diversity': ('django.db.models.fields.BinaryField', [], {'null': 'True', 'blank': 'True'}),
            'diversity_modified': ('django.db.models.fields.DateTimeField', [], {'null': 'True', 'blank': 'True'}),
            'email': ('django.db.models.fields.EmailField', [], {'max_length': '75', 'blank': 'True'}),
    ...
    complete_apps = ['cla_eventlog', 'cla_provider', 'legalaid']
    symmetrical = True
