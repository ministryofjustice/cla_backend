...
class ProviderExtractAPIMixin(object):
    ...
    def get_detail_url(self):
        return reverse("%s:provider-extract" % self.API_URL_NAMESPACE, args=())
    ...
    def test_post_to_get_extract_malformed_bad_request(self):
        response = self.client.post(self.detail_url, data=self.get_valid_post_data())
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_post_with_bad_creds_not_allowed(self):
        response = self.client.post(self.detail_url, data=self.get_valid_post_data(CHSOrganisationID="foofoofoo"))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

        response = self.client.post(self.detail_url, data=self.get_valid_post_data(CHSUserName="foofoofoo"))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

        response = self.client.post(self.detail_url, data=self.get_valid_post_data(CHSPassword="foofoofoo"))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_valid_but_legacy_post_to_extract_allowed(self):
        data = self.get_valid_post_data(CHSCRN=self.case.reference)
