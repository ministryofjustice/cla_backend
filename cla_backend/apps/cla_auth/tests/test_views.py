...
class LoginTestCase(TestCase):
    ...
    def test_username_does_not_exist(self):
        ...
        self.assert_unauthorised_error(data, '{"error": "invalid_grant"}')

    def test_user_does_not_exist(self):
        data = self.get_provider_data(username="user-does-not-exist")

        self.assert_unauthorised_error(data, '{"error": "invalid_client"}')

    def assert_unauthorised_error(self, data, expected_error):
        response = self.client.post(self.url, data=data)
        self.assertEqual(response.status_code, 401)
