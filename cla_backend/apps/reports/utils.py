...
class OBIEEExporter(object):
    ...
    no_timestamp_sql_files = ["export_auth_group.sql", "export_auth_user_groups.sql", "export_media_code_group.sql"]
    ...
    def __init__(self, export_path, passphrase, dt_from=None, dt_to=None, filename=None):
        if filename:
            self.filename = filename
        self.export_path = export_path
        self.passphrase = passphrase
        self.dt_from = dt_from or ((datetime.combine(date.today(), time.min) - timedelta(days=1)).isoformat())
        self.dt_to = dt_to or (datetime.combine(date.today(), time.min).isoformat())
    ...
    def export_personal_details(self):
        ...
        with open(sql_path, "r") as f:
            query = f.read()
            de = "pgp_pub_decrypt(diversity, dearmor('{key}'), %(passphrase)s)::json".format(
                key=diversity.get_private_key()
            )
            query = query.format(diversity_expression=de)

        csv_filename = self.csv_filename_from_sql_path(self.personal_details_sql_file)
        kwargs = {"passphrase": self.passphrase, "from_date": self.dt_from, "to_date": self.dt_to}
