...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.from_time = self._parse_date(args[0]) if len(args) >= 1 else \
            self._date_to_datetime(datetime.date.today() - datetime.timedelta(weeks=4))
        self.to_time = self._parse_date(args[1]) if len(args) >= 2 else \
            self._date_to_datetime(datetime.date.today())
        ...
        eligibility_checks = self._get_eligibility_checks().filter(
            state=ELIGIBILITY_STATES.NO
        )
        ...
        ecs = self._get_eligibility_checks().filter(
            Q(state=ELIGIBILITY_STATES.NO) & Q(case__isnull=True)
        )
        diags = self._get_diagnosis_traversals().filter(
            Q(case__isnull=True) & Q(state=DIAGNOSIS_SCOPE.OUTOFSCOPE)
        )
