...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if self.should_run_housekeeping(**options):
            ...
            logger.info('LGA-125: Skip remove_same_day_consecutive_outcome_codes because running on secondary instance')
    ...
    def remove_same_day_consecutive_outcome_codes(self):
        ...
        for e in cases_with_daily_multiple_events:
            ...
            logger.info('\nLGA-125: Case {} outcome log ids for {}: {}'.
                        format(e['case__reference'], e['day'], case_outcomes_for_day.values_list('id', flat=True)))
            for index, event in enumerate(case_outcomes_for_day):
                ...
                if index < n - 1 and case_outcomes_for_day[index + 1].code == event.code:
                    ...
                    logger.info(
                        'LGA-125: {:<7} {:<7} remove: {}'.format(event.id, event.code, event.created))
                else:
                    logger.info(
                        'LGA-125: {:<7} {:<7} keep:   {}'.format(event.id, event.code, event.created))
        ...
        if same_day_consecutive_outcome_log_ids:
            ...
            try:
                ...
                logger.error('LGA-125: Could not get bucket {}: {}'.format(settings.AWS_DELETED_OBJECTS_BUCKET_NAME, e))
            else:
