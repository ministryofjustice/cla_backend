...
class OperatorAdminForm(forms.ModelForm):
    ...
    def __init__(self, data=None, files=None, **kwargs):
        ...
        super(OperatorAdminForm, self).__init__(data=data, files=files, **kwargs)
    ...
    def save(self, commit=True):
        operator = super(OperatorAdminForm, self).save(commit=False)

        if not self.instance.pk:
            operator.user = User()

        operator.user.username = self.cleaned_data['username']
        operator.user.first_name = self.cleaned_data['first_name']
        operator.user.last_name = self.cleaned_data['last_name']
        operator.user.email = self.cleaned_data['email']

        if not self.instance.pk:
            operator.user.set_password(self.cleaned_data["password"])
