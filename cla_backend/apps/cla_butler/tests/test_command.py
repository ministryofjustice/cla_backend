...
class DiversityReencryptCommandTestCase(CreateSampleDiversityData, TestCase):
    @mock.patch("cla_butler.management.commands.diversity_reencrypt.Command.get_old_key_passphrase", return_value="cla")
    ...
    def test_create_tasks_some(self, mock_schedule_tasks, get_old_key_passphrase):
        ...
        DiversityDataCheck.objects.create(personal_details_id=self.pd_records_ids[0], action=ACTION.REENCRYPT, status=STATUS.OK)
