...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        clogtypes = []
        for outcome_code in orm.OutcomeCode.objects.all():
            clogtype = orm.CaseLogType(
                code=outcome_code.code,
                description=outcome_code.description,
                case_state=outcome_code.case_state,
                subtype='outcome'
            )
            clogtypes.append(clogtype)

        orm.CaseLogType.objects.bulk_create(clogtypes)

        clogs = []
        for case_outcome in orm.CaseOutcome.objects.all():
            clog = orm.CaseLog()
            clog.case = case_outcome.case
            clog.created_by = case_outcome.created_by
            clog.notes = case_outcome.notes
            clogtype= orm.CaseLogType.objects.get(code=case_outcome.outcome_code.code)
            clog.logtype = clogtype
            clogs.append(clog)

        orm.CaseLog.objects.bulk_create(clogs)

        orm.CaseOutcome.objects.all().delete()
        orm.OutcomeCode.objects.all().delete()

    def backwards(self, orm):
        outcome_codes = []
        for clogtype in orm.CaseLogType.objects.all():
            oc = orm.OutcomeCode(
                code=clogtype.code,
                description=clogtype.description,
                case_state=clogtype.case_state,
                )
            outcome_codes.append(oc)
        orm.OutcomeCode.objects.bulk_create(outcome_codes)

        case_outcomes = []
        for clog in orm.CaseLog.objects.all():
            co = orm.CaseOutcome(
                outcome_code=orm.OutcomeCode.get(code=clog.outcome_code),
                case=clog.case,
                created_by=clog.created_by,
                notes=clog.notes
            )
            case_outcomes.append(co)
        orm.CaseOutcome.objects.bulk_create(case_outcomes)
    ...
    models = {
        ...
        u'legalaid.caselogtype': {
            ...
            'case_state': ('django.db.models.fields.PositiveSmallIntegerField', [], {'null': 'True', 'blank': 'True'}),
        ...
        u'legalaid.caseoutcome': {
            'Meta': {'ordering': "['-created']", 'object_name': 'CaseOutcome'},
            'case': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['legalaid.Case']"}),
            'created': ('model_utils.fields.AutoCreatedField', [], {'default': 'datetime.datetime.now'}),
            'created_by': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['auth.User']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            ...
            'outcome_code': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['legalaid.OutcomeCode']"})
        },
        u'legalaid.category': {
        ...
        u'legalaid.outcomecode': {
            'Meta': {'ordering': "['code']", 'object_name': 'OutcomeCode'},
            'case_state': ('django.db.models.fields.PositiveSmallIntegerField', [], {'null': 'True', 'blank': 'True'}),
            'code': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '50'}),
            'created': ('model_utils.fields.AutoCreatedField', [], {'default': 'datetime.datetime.now'}),
            'description': ('django.db.models.fields.TextField', [], {}),
