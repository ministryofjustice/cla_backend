...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.add_column(u'legalaid_category', 'code',
                      self.gf('django.db.models.fields.CharField')(default='test', max_length=50),
                      keep_default=False)

        # Updating existing codes to random string
        for category in orm.Category.objects.all():
            category.code = "".join([random.choice(string.letters) for i in range(5)])
            category.save()

        # Making category code unique
